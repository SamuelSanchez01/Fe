...
var import_json_schema_to_zod = require("@bpinternal/json-schema-to-zod");
...
var import_memoize = __toESM(require("lodash/memoize"), 1);
var import_isPlainObject = __toESM(require("lodash/isPlainObject"), 1);
var Parent = Symbol("Parent");
var getRootSchema = (0, import_memoize.default)((schema) => {
...
function isPrimitive(schema) {
  return !(0, import_isPlainObject.default)(schema);
}
...
function toSafeString(string) {
  return (0, import_lodash.upperFirst)(
    // remove accents, umlauts, ... by their basic latin letters
    (0, import_lodash.deburr)(string).replace(/(^\s*[^a-zA-Z_$])|([^a-zA-Z_$\d])/g, " ").replace(/^_[a-z]/g, (match) => match.toUpperCase()).replace(/_[a-z]/g, (match) => match.substr(1, match.length).toUpperCase()).replace(/([\d$]+[a-zA-Z])/g, (match) => match.toUpperCase()).replace(/\s+([a-zA-Z])/g, (match) => (0, import_lodash.trim)(match.toUpperCase())).replace(/\s/g, "")
  );
}
...
function maybeStripDefault(schema) {
  ...
  switch (schema.type) {
    ...
    case "null":
      ...
      break;
    case "object":
      if ((0, import_lodash.isPlainObject)(schema.default)) {
        return schema;
      }
      break;
  }
...
function isSchemaLike(schema) {
  if (!(0, import_lodash.isPlainObject)(schema)) {
    return false;
  }
  const parent = schema[Parent];
  if (parent === null) {
...
var generateType = (0, import_lodash2.memoize)(generateTypeUnmemoized);
...
function generateStandaloneType(ast, options) {
  return (hasComment(ast) ? generateComment(ast.comment) + "\n" : "") + `export type ${toSafeString(ast.standaloneName)} = ${generateType(
    (0, import_lodash2.omit)(ast, "standaloneName"),
    options
  )}`;
}
...
function link(schema, parent = null) {
  if (!Array.isArray(schema) && !(0, import_isPlainObject2.default)(schema)) {
    return schema;
  }
...
rules.set("Add an $id to anything that needs it", (schema, fileName, _options, _key, dereferencedPaths) => {
  ...
  if (!schema.$id && !schema[Parent]) {
    ...
    id = (0, import_isEmpty.default)(id) ? "Root" : id;
...
rules.set("Transform definitions to $defs", (schema, fileName) => {
  if (schema.definitions && schema.$defs && !(0, import_isEqual.default)(schema.definitions, schema.$defs)) {
    throw ReferenceError(
...
var import_uniqBy = __toESM(require("lodash/uniqBy"), 1);
function optimize(ast, options, processed = /* @__PURE__ */ new Set()) {
  ...
  switch (ast.type) {
    ...
    case "UNION":
      ...
      const params = (0, import_uniqBy.default)(optimizedAST.params, (_) => generateType(_, options));
...
var matchers = {
  ...
  OBJECT(schema) {
    return schema.type === "object" && !(0, import_isPlainObject3.default)(schema.additionalProperties) && !schema.allOf && !schema.anyOf && !schema.oneOf && !schema.patternProperties && !schema.properties && !schema.required;
  },
  ONE_OF(schema) {
...
function parseNonLiteral(schema, type, options, keyName, processed, usedNames) {
  ...
  const keyNameFromDefinition = (0, import_lodash3.findKey)(definitions, (_) => _ === schema);
  switch (type) {
    case "ALL_OF":
      return {
    ...
    case "UNION":
      return {
        ...
        params: schema.type.map((type2) => {
          const member = __spreadProps(__spreadValues({}, (0, import_lodash3.omit)(schema, "$id", "description", "title")), { type: type2 });
...
function parseSchema(schema, options, processed, usedNames, parentSchemaName) {
  let asts = (0, import_lodash3.map)(schema.properties, (value, key) => ({
    ast: parse(value, options, key, processed, usedNames),
    isPatternProperty: false,
    isRequired: (0, import_lodash3.includes)(schema.required || [], key),
  ...
  if (schema.patternProperties) {
    ...
    asts = asts.concat(
      (0, import_lodash3.map)(schema.patternProperties, (value, key) => {
...
${comment}` : comment;
        return {
          ...
          isRequired: singlePatternProperty || (0, import_lodash3.includes)(schema.required || [], key),
          isUnreachableDefinition: false,
  ...
  if (options.unreachableDefinitions) {
    asts = asts.concat(
      (0, import_lodash3.map)(schema.$defs, (value, key) => {
        const ast = parse(value, options, key, processed, usedNames);
...
function getDefinitions(schema, isSchema = true, processed = /* @__PURE__ */ new Set()) {
  ...
  if ((0, import_lodash3.isPlainObject)(schema)) {
...
var getDefinitionsMemoized = (0, import_lodash3.memoize)(getDefinitions);
...
async function compile(schema, name = "Root", options = {}) {
  ...
  const _options = (0, import_lodash4.merge)({}, DEFAULT_OPTIONS, options);
  const start = Date.now();
  function time() {
    return `(${Date.now() - start}ms)`;
  }
  __name(time, "time");
  if (!(0, import_lodash4.endsWith)(_options.cwd, "/")) {
    _options.cwd += "/";
  }
  const _schema = (0, import_lodash4.cloneDeep)(schema);
  const { dereferencedPaths, dereferencedSchema } = await dereference(_schema, _options);
  if (process.env.VERBOSE) {
    if ((0, import_lodash4.isEqual)(_schema, dereferencedSchema)) {
...
var jsonSchemaToZodStr = /* @__PURE__ */ __name((schema) => {
  return (0, import_json_schema_to_zod.parseSchema)(schema, {
...
var useFormData = /* @__PURE__ */ __name(() => {
  const context = (0, import_react.useContext)(FormDataContext);
  if (context === void 0) {
...
var FormDataProvider = /* @__PURE__ */ __name(({
  ...
  return /* @__PURE__ */ import_react2.default.createElement(FormDataContext.Provider, { value: { formData, setFormData, formSchema, disableValidation } }, children);
}, "FormDataProvider");
...
var FormElementRenderer = /* @__PURE__ */ __name((_a) => {
  ...
  const data = (0, import_react3.useMemo)(() => getPathData(formData, path), [formData, path]);
  const componentMeta = (0, import_react3.useMemo)(() => resolveComponent(components, fieldSchema), [fieldSchema, components]);
  if (!componentMeta) {
    return null;
  }
  ...
  if (fieldSchema.type === "array" && type === "array") {
    ...
    return /* @__PURE__ */ import_react3.default.createElement(Component2, __spreadProps(__spreadValues({ key: baseProps.scope }, props2), { isArrayChild: props2.isArrayChild }), ((_e = props2.data) == null ? void 0 : _e.map((_, index) => {
  ...
  if (fieldSchema.type === "object" && type === "object") {
    ...
    return /* @__PURE__ */ import_react3.default.createElement(Component2, __spreadProps(__spreadValues({ key: baseProps.scope }, props2), { isArrayChild: props2.isArrayChild }), Object.entries(fieldSchema.properties).map(([fieldName, childSchema]) => {
      var _a3;
  ...
  return /* @__PURE__ */ import_react3.default.createElement(Component, __spreadValues({}, props));
}, "FormElementRenderer");
