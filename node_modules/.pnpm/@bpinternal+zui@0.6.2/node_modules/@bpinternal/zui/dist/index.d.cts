...
type ToTypescriptTyingsOptions = {
    schemaName: string;
} & Partial<Options$1>;
...
declare const jsonSchemaToZui: (schema: JsonSchema7Type | any) => ZodTypeAny;
...
type PrimitiveSchema = StringSchema | NumberSchema | BooleanSchema;
type JSONSchema = ArraySchema | ObjectSchema | PrimitiveSchema;
type JSONSchemaOfType<T extends BaseType> = T extends 'string' ? StringSchema : T extends 'number' ? NumberSchema : T extends 'boolean' ? BooleanSchema : T extends 'object' ? ObjectSchema : T extends 'array' ? ArraySchema : never;
type BaseType = 'number' | 'string' | 'boolean' | 'object' | 'array';
declare const containerTypes: readonly ["object", "array"];
type ContainerType = (typeof containerTypes)[number];
type DefaultComponentDefinitions = {
    number: {};
    string: {};
    boolean: {};
    object: {};
...
type ZodKindToBaseType<T extends ZodTypeDef> = T extends infer U ? U extends {
    ...
    innerType: ZodTypeAny;
} ? ZodKindToBaseType<U['innerType']['_def']> : never : never;
type BaseTypeToType<T extends BaseType> = T extends 'string' ? string : T extends 'number' ? number : T extends 'boolean' ? boolean : T extends 'object' ? object : T extends 'array' ? any[] : never;
type AsBaseType<T> = T extends BaseType ? T : never;
type FormError = {
    message: string;
    path: (string | number)[];
};
type ZuiReactComponentBaseProps<Type extends BaseType, ID extends keyof UI[Type], UI extends UIComponentDefinitions = DefaultComponentDefinitions> = {
    type: Type;
    componentID: ID;
    params: ID extends 'default' ? {} : TypeOf<UI[Type][ID]['params']>;
...
type ZuiReactObjectComponentProps<Type extends ContainerType, ID extends keyof UI[Type], UI extends UIComponentDefinitions = DefaultComponentDefinitions> = ZuiReactComponentBaseProps<Type, ID, UI> & {
    children: JSX.Element | JSX.Element[];
};
type ZuiReactArrayComponentProps<Type extends ContainerType, ID extends keyof UI[Type], UI extends UIComponentDefinitions = DefaultComponentDefinitions> = ZuiReactComponentBaseProps<Type, ID, UI> & {
    children: JSX.Element | JSX.Element[];
    addItem: (initialData?: any) => void;
    removeItem: (index: number) => void;
};
type ZuiReactControlComponentProps<Type extends BaseType, ID extends keyof UI[Type], UI extends UIComponentDefinitions = DefaultComponentDefinitions> = ZuiReactComponentBaseProps<Type, ID, UI> & {
    description?: string;
    required: boolean;
    config: any;
};
type ZuiReactComponentProps<Type extends BaseType, ID extends keyof UI[Type], UI extends UIComponentDefinitions = DefaultComponentDefinitions> = Type extends 'object' ? ZuiReactObjectComponentProps<Type, ID, UI> : Type extends 'array' ? ZuiReactArrayComponentProps<Type, ID, UI> : ZuiReactControlComponentProps<Type, ID, UI>;
type ZuiReactComponent<Type extends BaseType, ID extends keyof UI[Type], UI extends UIComponentDefinitions = DefaultComponentDefinitions> = FC<ZuiReactComponentProps<Type, ID, UI>>;
type ZuiComponentMap<UI extends UIComponentDefinitions = DefaultComponentDefinitions> = {
    [T in BaseType]: {
...
declare const zuiToJsonSchema: (zuiType: ZodTypeAny, opts?: ZuiSchemaOptions) => JSONSchema;
...
declare function processCreateParams(params: RawCreateParams): ProcessedCreateParams;
...
declare abstract class ZodType<Output = any, Def extends ZodTypeDef = ZodTypeDef, Input = Output> {
     ...
     */
    displayAs<UI extends UIComponentDefinitions = DefaultComponentDefinitions, Type extends BaseType = ZodKindToBaseType<this['_def']>>(options: ParseSchema<UI[Type][keyof UI[Type]]>): this;
    /**
    ...
    toTypescriptTypings(opts?: ToTypescriptTyingsOptions): Promise<string>;
    static fromObject(obj: any, opts?: ObjectToZuiOptions): ZodTypeAny;
    static fromJsonSchema(schema: JSONSchema | any): ZodTypeAny;
}

type ZodDef = ZodStringDef | ZodNumberDef | ZodBigIntDef | ZodBooleanDef | ZodDateDef | ZodUndefinedDef | ZodNullDef | ZodAnyDef | ZodUnknownDef | ZodNeverDef | ZodVoidDef | ZodArrayDef | ZodObjectDef | ZodUnionDef | ZodIntersectionDef | ZodTupleDef | ZodRecordDef | ZodMapDef | ZodFunctionDef | ZodLazyDef | ZodLiteralDef | ZodEnumDef | ZodEffectsDef | ZodNativeEnumDef | ZodOptionalDef | ZodNullableDef | ZodPromiseDef;
...
declare class ZodTemplateLiteralUnsupportedTypeError extends Error {
    constructor();
}
declare class ZodTemplateLiteralUnsupportedCheckError extends Error {
...
declare const custom: <T>(check?: (data: unknown) => any, params?: string | CustomParams | ((input: any) => CustomParams), fatal?: boolean) => ZodType<T>;
...
declare class ZodObject<T extends ZodRawShape, UnknownKeys extends UnknownKeysParam = UnknownKeysParam, Catchall extends ZodTypeAny = ZodTypeAny, Output = objectOutputType<T, Catchall, UnknownKeys>, Input = objectInputType<T, Catchall, UnknownKeys>> extends ZodType<Output, ZodObjectDef<T, UnknownKeys, Catchall>, Input> {
    ...
    static create: <T_1 extends ZodRawShape>(shape: T_1, params?: RawCreateParams) => ZodObject<T_1, "strip", ZodTypeAny, objectUtil.addQuestionMarks<baseObjectOutputType<T_1>, { [k_1 in keyof baseObjectOutputType<T_1>]: undefined extends baseObjectOutputType<T_1>[k_1] ? never : k_1; }[keyof T_1], { [k_2 in keyof baseObjectOutputType<T_1>]: undefined extends baseObjectOutputType<T_1>[k_2] ? k_2 : never; }[keyof T_1]> extends infer T_2 ? { [k in keyof T_2]: objectUtil.addQuestionMarks<baseObjectOutputType<T_1>, { [k_1 in keyof baseObjectOutputType<T_1>]: undefined extends baseObjectOutputType<T_1>[k_1] ? never : k_1; }[keyof T_1], { [k_2 in keyof baseObjectOutputType<T_1>]: undefined extends baseObjectOutputType<T_1>[k_2] ? k_2 : never; }[keyof T_1]>[k]; } : never, baseObjectInputType<T_1> extends infer T_3 ? { [k_3 in keyof T_3]: baseObjectInputType<T_1>[k_3]; } : never>;
...
declare class ZodEffects<T extends ZodTypeAny, Output = output<T>, Input = input<T>> extends ZodType<Output, ZodEffectsDef<T>, Input> {
    ...
    sourceType(): T;
    _parse(input: ParseInput): ParseReturnType<this['_output']>;
    static create: <I extends ZodType<any, ZodTypeDef, any>>(schema: I, effect: Effect<I['_output']>, params?: RawCreateParams) => ZodEffects<I, I['_output']>;
...
declare const datetimeRegex: (args: {
    precision: number | null;
    offset: boolean;
}) => RegExp;
...
declare class ZodTemplateLiteral<Template extends string = ''> extends ZodType<Template, ZodTemplateLiteralDef> {
    interpolated<I extends TemplateLiteralInterpolatedPosition>(type: Exclude<I, ZodNever | ZodNaN | ZodPipeline<any, any> | ZodLazy<any>>): ZodTemplateLiteral<appendToTemplateLiteral<Template, I>>;
    ...
    protected _appendToRegexString(regexString: string, part: TemplateLiteralPrimitive | TemplateLiteralInterpolatedPosition): string;
...
declare const effectsType: <I extends ZodType<any, ZodTypeDef, any>>(schema: I, effect: Effect<I["_output"]>, params?: RawCreateParams) => ZodEffects<I, I["_output"], input<I>>;
...
declare const templateLiteralType: (params?: ({
    errorMap?: ZodErrorMap | undefined;
    invalid_type_error?: string | undefined;
    required_error?: string | undefined;
    description?: string | undefined;
    ...
    coerce?: true | undefined;
}) | undefined) => ZodTemplateLiteral<"">;
declare const ostring: () => ZodOptional<ZodString>;
declare const onumber: () => ZodOptional<ZodNumber>;
declare const oboolean: () => ZodOptional<ZodBoolean>;
declare const fromJsonSchema: typeof ZodType.fromJsonSchema;
declare const fromObject: typeof ZodType.fromObject;
...
type z_ZodTemplateLiteral<Template extends string = ''> = ZodTemplateLiteral<Template>;
declare const z_ZodTemplateLiteral: typeof ZodTemplateLiteral;
type z_ZodTemplateLiteralDef = ZodTemplateLiteralDef;
type z_ZodTemplateLiteralUnsupportedCheckError = ZodTemplateLiteralUnsupportedCheckError;
declare const z_ZodTemplateLiteralUnsupportedCheckError: typeof ZodTemplateLiteralUnsupportedCheckError;
type z_ZodTemplateLiteralUnsupportedTypeError = ZodTemplateLiteralUnsupportedTypeError;
declare const z_ZodTemplateLiteralUnsupportedTypeError: typeof ZodTemplateLiteralUnsupportedTypeError;
...
declare const z_fromObject: typeof fromObject;
...
declare const z_jsonSchemaToZui: typeof jsonSchemaToZui;
...
declare const z_objectToZui: typeof objectToZui;
...
declare const objectToZui: (obj: any, opts?: ObjectToZuiOptions, isRoot?: boolean) => ZodTypeAny;

type ZuiFormProps<UI extends UIComponentDefinitions = DefaultComponentDefinitions> = {
    schema: JSONSchema;
...
declare const ZuiForm: <UI extends UIComponentDefinitions = DefaultComponentDefinitions>({ schema, components, onChange, value, disableValidation, }: ZuiFormProps<UI>) => JSX.Element | null;

declare const transforms: {
    jsonSchemaToZui: (schema: any) => ZodTypeAny;
    zuiToJsonSchema: (zuiType: ZodTypeAny, opts?: ZuiSchemaOptions) => JSONSchema;
    objectToZui: (obj: any, opts?: ObjectToZuiOptions | undefined, isRoot?: boolean) => ZodTypeAny;
    zuiToTypescriptTypings: (jsonSchema: any, options?: ToTypescriptTyingsOptions | undefined) => Promise<string>;
};

export { type AnyZodObject, type AnyZodTuple, type ArrayCardinality, type ArrayKeys, type AsBaseType, type AssertArray, type AsyncParseReturnType, BRAND, type BaseType, type CatchallInput, type CatchallOutput, type CustomErrorParams, type CustomParams, DIRTY, type DefaultComponentDefinitions, type DenormalizedError, EMPTY_PATH, type Effect, type EnumLike, type EnumValues, type ErrorMapCtx, type FilterEnum, INVALID, type Indices, type InnerTypeOfFunction, type InputTypeOfTuple, type InputTypeOfTupleWithRest, type IpVersion, type IssueData, type KeySchema, type KindToDef, NEVER, OK, type ObjectPair, type OuterTypeOfFunction, type OutputTypeOfTuple, type OutputTypeOfTupleWithRest, type ParseContext, type ParseInput, ParseInputLazyPath, type ParseParams, type ParsePath, type ParsePathComponent, type ParseResult, type ParseReturnType, ParseStatus, type PassthroughType, type PreprocessEffect, type Primitive, type ProcessedCreateParams, type RawCreateParams, type RecordType, type Refinement, type RefinementCtx, type RefinementEffect, type SafeParseError, type SafeParseReturnType, type SafeParseSuccess, type Scalars, ZodType as Schema, type SomeZodObject, type StringValidation, type SuperRefinement, type SyncParseReturnType, type TransformEffect, type TypeOf, type UIComponentDefinitions, type UnknownKeysParam, type Values, type Writeable, ZodAny, type ZodAnyDef, ZodArray, type ZodArrayDef, ZodBigInt, type ZodBigIntCheck, type ZodBigIntDef, ZodBoolean, type ZodBooleanDef, ZodBranded, type ZodBrandedDef, ZodCatch, type ZodCatchDef, type ZodCustomIssue, ZodDate, type ZodDateCheck, type ZodDateDef, type ZodDef, ZodDefault, type ZodDefaultDef, ZodDiscriminatedUnion, type ZodDiscriminatedUnionDef, type ZodDiscriminatedUnionOption, ZodEffects, type ZodEffectsDef, ZodEnum, type ZodEnumDef, ZodError, type ZodErrorMap, type ZodFirstPartySchemaTypes, ZodFirstPartyTypeKind, type ZodFormattedError, ZodFunction, type ZodFunctionDef, ZodIntersection, type ZodIntersectionDef, type ZodInvalidArgumentsIssue, type ZodInvalidDateIssue, type ZodInvalidEnumValueIssue, type ZodInvalidIntersectionTypesIssue, type ZodInvalidLiteralIssue, type ZodInvalidReturnTypeIssue, type ZodInvalidStringIssue, type ZodInvalidTypeIssue, type ZodInvalidUnionDiscriminatorIssue, type ZodInvalidUnionIssue, type ZodIssue, type ZodIssueBase, ZodIssueCode, type ZodIssueOptionalMessage, ZodLazy, type ZodLazyDef, ZodLiteral, type ZodLiteralDef, ZodMap, type ZodMapDef, ZodNaN, type ZodNaNDef, ZodNativeEnum, type ZodNativeEnumDef, ZodNever, type ZodNeverDef, type ZodNonEmptyArray, type ZodNotFiniteIssue, type ZodNotMultipleOfIssue, ZodNull, type ZodNullDef, ZodNullable, type ZodNullableDef, type ZodNullableType, ZodNumber, type ZodNumberCheck, type ZodNumberDef, ZodObject, type ZodObjectDef, ZodOptional, type ZodOptionalDef, type ZodOptionalType, ZodParsedType, ZodPipeline, type ZodPipelineDef, ZodPromise, type ZodPromiseDef, type ZodRawShape, ZodReadonly, type ZodReadonlyDef, ZodRecord, type ZodRecordDef, ZodType as ZodSchema, ZodSet, type ZodSetDef, ZodString, type ZodStringCheck, type ZodStringDef, ZodSymbol, type ZodSymbolDef, ZodTemplateLiteral, type ZodTemplateLiteralDef, ZodTemplateLiteralUnsupportedCheckError, ZodTemplateLiteralUnsupportedTypeError, type ZodTooBigIssue, type ZodTooSmallIssue, ZodEffects as ZodTransformer, ZodTuple, type ZodTupleDef, type ZodTupleItems, ZodType, type ZodTypeAny, type ZodTypeDef, ZodUndefined, type ZodUndefinedDef, ZodUnion, type ZodUnionDef, type ZodUnionOptions, ZodUnknown, type ZodUnknownDef, type ZodUnrecognizedKeysIssue, ZodVoid, type ZodVoidDef, type ZuiComponentMap, ZuiForm, type ZuiFormProps, type ZuiReactComponent, type ZuiReactComponentProps, addIssueToContext, anyType as any, arrayType as array, type arrayOutputType, type baseObjectInputType, type baseObjectOutputType, bigIntType as bigint, booleanType as boolean, coerce, createZodEnum, cuid2Regex, cuidRegex, custom, dateType as date, datetimeRegex, errorMap as defaultErrorMap, type deoptional, discriminatedUnionType as discriminatedUnion, effectsType as effect, emailRegex, enumType as enum, enumUtil, errorUtil, fromJsonSchema, fromObject, functionType as function, getErrorMap, getParsedType, type TypeOf as infer, type inferFlattenedErrors, type inferFormattedError, type input, instanceOfType as instanceof, intersectionType as intersection, ipv4Regex, ipv6Regex, isAborted, isAsync, isDirty, isValid, jsonSchemaToZui, late, lazyType as lazy, literalType as literal, makeIssue, mapType as map, type mergeTypes, nanType as nan, nativeEnumType as nativeEnum, neverType as never, type noUnrecognized, nullType as null, nullableType as nullable, numberType as number, objectType as object, type objectInputType, type objectOutputType, objectToZui, objectUtil, oboolean, onumber, optionalType as optional, ostring, type output, partialUtil, pipelineType as pipeline, preprocessType as preprocess, processCreateParams, promiseType as promise, quotelessJson, recordType as record, setType as set, setErrorMap, strictObjectType as strictObject, stringType as string, symbolType as symbol, templateLiteralType as templateLiteral, effectsType as transformer, transforms, tupleType as tuple, type typeToFlattenedError, type typecast, ulidRegex, undefinedType as undefined, unionType as union, unknownType as unknown, util, uuidRegex, voidType as void, z, zuiToJsonSchema };
